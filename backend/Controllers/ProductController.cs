...
namespace SweetShop.Controllers
{
    ...
    public class ProductController : ControllerBase
    {
        ...
        public ProductDetails Get(Guid productid)
        {
            ...
            try
            {
                rating = db.GetRaiting(productid);
            }
            catch{}
        ...
        //TODO: logic to add 2 or more tags
        [HttpPost("create")]
        public IActionResult Add(Guid userid, string name, string description, decimal price, string url, string token, Guid categoryId, Guid tagsid)
        {
            try
            {
                var product = db.AddProduct(name, description, price);
                Console.WriteLine(url + "&token=" + token);
                db.AddImage(url+"&token="+token, product);
                ...
                db.AddProductTag(tagsid, product);
                // foreach (var item in tagsid)
                // {
                //     db.AddProductTag(item, shopid);
                // }
            }
